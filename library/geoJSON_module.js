...
const buildGeoJSONFromJSON = (name, description, lat, lng) => {
  const convertedData = {
    "type": "Feature",
    "properties": {
        "title": name,
...
//call the convertToGeoJson(api_result)
const convertToGeoJSON = (processedAPIResult) => {
  
  //make the geoJson frame
  const geoJSONObject = {
    "type": "FeatureCollection"
  }
  
  // make an array (collection)
  let collectionArray = []
  
  // process to convert the data and push in the array
  processedAPIResult.data.places.forEach(poi => {
    collectionArray.push(buildGeoJSONFromJSON(poi.name, poi.perex, poi.location.lat, poi.location.lng));
  })
  
  // at the end push everything in the GEOJSON Object
  geoJSONObject.features = collectionArray
...
// module.exports.buildGeoJSONFromJSON = buildGeoJSONFromJSON;