...
const buildGeoJSONFromJSON = (id, name, description, lat, lng, image_url) => {
  const convertedData = {
    "type": "Feature",
    "properties": {
        "id": id,
...
const convertToGeoJSON = (processedAPIResult) => {
  ...
  processedAPIResult.data.places.forEach(poi => {
    collectionArray.push(buildGeoJSONFromJSON(poi.id, poi.name, poi.perex, poi.location.lat, poi.location.lng, poi.thumbnail_url));
  })
